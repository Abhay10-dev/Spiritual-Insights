    ...
    <div className="min-h-screen">
      ...
      <div className="gradient-spiritual px-4 pt-10 pb-14">
        <div className="mx-auto max-w-5xl">
          <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-3xl font-bold text-white mb-1">📹 Video Library</motion.h1>
      ...
      <div className="mx-auto max-w-5xl px-4 -mt-6 space-y-6 pb-10">
          ...
          {activeVideo && currentVideo && (
            ...
            >
              ...
              <div className="p-4 flex justify-between items-center">
                ...
                <button onClick={() => setActiveVideo(null)} className="rounded-lg border border-gray-200 px-3 py-1.5 text-sm text-gray-500 hover:bg-gray-50 transition-colors">Close</button>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <motion.div layout className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <AnimatePresence>
            {filtered.length === 0 ? (
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="col-span-full text-center text-gray-400 py-10">No videos found.</motion.p>
            ) : (
              filtered.map((v) => (
                <motion.div key={v.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <VideoCard {...v} onClick={(id) => setActiveVideo(id)} />
                </motion.div>
              ))
            )}
          </AnimatePresence>
